...
class RecAgent(GenerativeAgent):
    ...
    watched_history: List[str] = Field(default_factory=list)
    """The agent's history of watched movies"""

    heared_history: List[str] = Field(default_factory=list)
    ...
    def from_roleagent(cls, roleagent_instance: "RecAgent"):
        ...
        new_instance = cls(
            ...
            chatting_url=roleagent_instance.chatting_url,
    ...
    def select_interest(self, interest_pool, inverse_selection=False):
        """
        Select the interest of the agent.
        """
        prompt = PromptTemplate.from_template(
            """You are {agent_name}, with the following profile:  
- **Name:** {agent_name}  
- **Age:** {agent_age}  
- **Gender:** {agent_gender}  
- **Traits:** {agent_traits}  
- **Status:** {agent_status}  
- **Previously Watched Movie Types:** {agent_interest}  
- **Feature:** {agent_feature}  
- **Interpersonal Relationships:** {agent_relationships}  
Now, you are registering on a movie website. The system displays some interest tags `{interests}` on the welcome page. {initial_status}
**Task:**  
- {select_prompt}
**Instructions:**  
- {select_instruction}
- Your final selection must align with your profile.  
- You should only output selected types of movies without any additional output.
- You should keep at least one type of movie selected.
**Your output**:
"""
#             """You are {agent_name}, your profile is:
# - Name: {agent_name}
# - Age: {agent_age}
# - Gender:{agent_gender}
# - Traits: {agent_traits}
...
# - Feature: {agent_feature}
# - Interpersonal Relationships: {agent_relationships}
# Now you are registering on a movie website, and the system shows `{interests}` on welcome page. They are initially {initial_selected}, please click on them to {select_prompt}. The final items selected should align with your profile. You should only output selected types of movies without any additional output. You should keep at least one type of movie selected.
# The item(s) in selected status:
# """
        )
        kwargs: Dict[str, Any] = dict(
            ...
            interests=','.join(interest_pool),
            agent_interest=self.interest,
            agent_feature=self.feature,
            agent_relationships=self.relationships,
            initial_status="" if not inverse_selection else "They are now all selected.",
            select_prompt="You need to select **at least one** movie type from the listed interests." if not inverse_selection else "You are initially shown all the movies as your selected interests, however you may not be interested in all of them. You need to **deselect** the types you are not interested in.",
            select_instruction="Focus on choosing a small subset of interests that best match your personality and preferences." if not inverse_selection else "Review the initial selections and remove at least one that do not reflect your actual tastes. Please tell me the *removed* types.",
            observation='initializing the agent'
        )
        result = self.chain(prompt=prompt).run(**kwargs).strip()
        return result

    def rate_tags(self, interest_pool):
        """
        Select the interest of the agent.
        """
        prompt = PromptTemplate.from_template(
...
- **Interpersonal Relationships:** {agent_relationships}
**Task**:
Consider a list of movie genres `{interests}`. For each genre, please answer the following two questions:
1) how fun is it to watch movies of this genre?
2) how beneficial is it to your life to watch movies of this genre?
Please consider them on a 100-point slider (0 = not at all, 100 = a lot of fun/benefit).
The rating should align with your profile.
Please provide your rating in the following JSON format: {{"fun": {{"genre1": score1, ...}}, "benefit": {{"genre1": score1, ...}}}}, with no additional commentary.
**Your output**:
"""
        )
        kwargs: Dict[str, Any] = dict(
    ...
    def _generate_reaction(
        ...
        prompt = PromptTemplate.from_template(
            ...
            + "\nObservation: {observation}"
            + "\nYou are interested in these types of movies: `{interest}`"
    ...
    def take_recommender_action(self, observation, now) -> Tuple[str, str]:
        ...
        call_to_action_template = (
            ...
            + "(4) Leave the recommender system."
            + "\n\nBehavior rules:"
            + "\n- By default, {agent_name} prefers to watch or search for movies that have NOT been watched before."
            + "\n- {agent_name} should avoid repeatedly watching or searching for the same movie in consecutive or recent rounds."
            + "\n- However, re-watching or re-searching a previously watched movie is allowed ONLY IF there is a clear and explicit reason, such as:"
            + "\n  (a) the movie was watched a long time ago,"
            + "\n  (b) the movie is one of {agent_name}'s favorites,"
            + "\n  (c) the movie is currently trending or frequently mentioned by friends on social media,"
            + "\n  (d) the movie has special relevance (e.g., sequel released, ongoing discussion with friends)."
            + "\n\nIf {agent_name} has recently heard about a particular movie on social media that "
            + "{agent_name} has not watched yet, or has a clear reason to revisit a known movie, "
            + "{agent_name} usually wants to search for that movie."
            + "\n\nTo watch a movie from the recommended list that matches {agent_name}'s interests, write:\n"
            ...
            + "\n\nImportant:"
            + "\n- Do NOT repeatedly watch or search for the same movie across multiple rounds unless a clear and explicit reason applies."
        )
        full_result = self._generate_reaction(observation, call_to_action_template, now)

        result = full_result.strip()
        if result.find("::") != -1:
            choice, action = result.split("::")[:2]
            choice = choice.strip()
    ...
    def take_social_action(self, observation, now) -> Tuple[str, str]:
        ...
        action = re.split(r"\s*\[", action)[0].strip()
