...
class SensoryMemory():
    ...
    def dump_shortTerm_list(self):
        ...
        result = LLMChain(llm=self.llm, prompt=prompt).run(message=obs_str + order_str)
...
class ShortTermMemory:
    ...
    def get_memory(self) -> str:
        ...
        """
        # Return only the most recent 10 short-term memories
        recent_memories = self.short_memories[-10:]
...
class LongTermMemory(BaseMemory):
    ...
    def get_memory(self) -> str:
        ...
        """
        # Return only the most recent 10 long-term memories
        recent_memories = self.memory_retriever.memory_stream[-10:]
        return " ".join([re.sub(r"^\[\d+\]\s*", "", mem.page_content).strip() for mem in recent_memories])
...
class RecAgentMemory(BaseMemory):
    ...
    def get_short_memory(self) -> str:
        ...
        :return: The short-term memory.
        """
        return self.shortTermMemory.get_memory()
    ...
    def get_long_memory(self) -> str:
        ...
        :return: The long-term memory.
        """
        return self.longTermMemory.get_memory()
